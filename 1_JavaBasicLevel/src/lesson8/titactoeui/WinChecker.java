...
import static lesson8.titactoeui.MapDirection.*;
...
public class WinChecker
{
    ...
    public boolean check(char symbol)
    {
        ...
        for (int i = 0; i < this.map.length; i++)
        {
            ...
            winCountBackDiagonal = this.checkMapSymbolForWinCount(this.map[i][this.getColumnNumber() - 1 - i], symbol, winCountBackDiagonal);
    ...
    public boolean checkAndWinOrBlock(char symbolToBlock, char blockSymbol)
    {
        ...
        for (int i = 0; i < this.map.length; i++)
        {
            ...
            winCountBackDiagonal += this.checkMapSymbolForTryWinCount(this.map[i][this.getColumnNumber() - 1 - i], symbolToBlock);
    ...
    private boolean hasEmpty(MapDirection direction, int fixIdx)
    {
        for (int i = 0; i < this.map.length; i++)
        {
            ...
            switch (direction)
            {
                ...
                case BACK_DIAGONAL:
                    checkChar = this.map[i][this.getColumnNumber() - 1 - i];
                    break;
            }
